  ...
  <div>
    <el-card style="width: 98%; ">
      <el-form :inline="true" class="form">
        ...
        <el-form-item>
          <el-button type="primary" size="default" :disabled="keyword.length === 0" @click="searchRole">搜索</el-button>
    ...
    <el-card style="margin: 10px 0; width: 98%;">
      ...
      <el-table border style="width: 98%; margin: 10px 0;" :data="roleList">
        ...
        <el-table-column label="职位名称" prop="roleName" align="center" show-overflow-tooltip v-model="roleParams"></el-table-column>
        ...
        <el-table-column label="操作" width="280">
          <template #default="{row}">
            <el-button type="primary" size="small" icon="User" @click="setPermission(row)" title="分配权限"></el-button>
              <el-button type="warning" size="small" icon="Edit" title="编辑" @click="updateRole(row)"></el-button>
                             <el-popconfirm :title="`确定删除${row.roleName}吗？`" @confirm="deleteRole(row)">
                 <template #reference>
    ...
    <el-dialog v-model="dialogVisible" :title="roleParams.role_id ? '编辑职位' : '添加职位'" width="30%">
      <el-form  label-width="120px" :model="roleParams" :rules="roleRules"  ref="roleFormRef">
...
import { reqRoleList,reqAddOrUpdateRole,reqAllMenuList,reqSetPermission ,reqDeleteRole} from '../../../api/acl/role'
...
const roleRules = reactive<any>({
  roleName: [
    { required: true, message: '请输入职位名称', trigger: 'blur' ,validator: validateRoleName}
  ]
})
...
const getHasRole = async (pager: number = 1) => {
  ...
  console.log('获取角色列表参数:', { page: currentPage.value, limit: pageSize.value, keyword: keyword.value })
  
  const result: RoleResponseData = await reqRoleList(currentPage.value, pageSize.value, keyword.value)
...
const deleteRole = async (row: any) => {
 ...
 if(result.code == 200){
  ...
  getHasRole(menuArr.value.length>1?currentPage.value:currentPage.value-1)
 }else{
