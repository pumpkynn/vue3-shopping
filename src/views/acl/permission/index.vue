<template>
  <div>
    <el-table border style="width: 98%; margin: 10px 0;"  row-key="id" :data="permissionList" >
<el-table-column label="名称" prop="name" align="center" sortable ></el-table-column>
<el-table-column label="权限值" prop="code" align="center" sortable ></el-table-column>
<el-table-column label="级别" prop="level" align="center" sortable >
  <template #header>
    <el-tooltip content="1=顶级菜单，2=子菜单，3=子菜单的子菜单" placement="top">
      <span>级别 <el-icon><QuestionFilled /></el-icon></span>
    </el-tooltip>
  </template>
...
<el-table-column label="类型" prop="type" align="center" sortable >
  <template #header>
    <el-tooltip content="1=菜单权限，2=按钮权限，3=接口权限" placement="top">
      <span>类型 <el-icon><QuestionFilled /></el-icon></span>
    </el-tooltip>
  </template>
  <template #default="{ row }">
    <el-tag :type="row.type === 1 ? 'info' : row.type === 2 ? 'warning' : 'danger'">
...
<el-table-column label="操作" align="center" width="280">
  <template #default="{row}">
    <el-button type="primary" size="small" icon="Plus" :title="row.level == 3?'添加功能':'添加菜单'" 
     :disabled="row.level == 4?true:false" @click="addPermission(row)"></el-button>
    <el-button type="info" size="small" icon="Edit" title="编辑"  
    :disabled="row.level == 1?true:false" @click="updatePermission(row)"></el-button>
...
import type { PermissionList ,PermissionResponseData,MenuParams} from '../../../api/acl/menu/type'
