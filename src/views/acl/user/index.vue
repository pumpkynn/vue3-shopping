<template>
  ...
  <el-card style="width:98%">
    <el-form :inline="true" class="form">
      ...
      <el-form-item>
      <el-button type="primary" size="default" :disabled="keyword.length === 0" @click="searchUser()">搜索</el-button>
      <el-button type="default" size="default" @click="refresh">重置</el-button>
    </el-form-item>
    </el-form>
  ...
  <el-card style="margin: 10px 0;width:98%" >
   ...
   <el-table style="margin: 10px 0;" border :data="userArr" @selection-change="selectChange">
    ...
    <el-table-column label="用户名字" prop="name" align="center"></el-table-column>
    <el-table-column label="用户名称" prop="username" align="center"></el-table-column>
        <el-table-column label="用户角色" width="180" align="center">
          <template #default="{row}">
            <div v-if="row.roleName && row.roleName.trim()" style="display: flex; flex-wrap: wrap; gap: 4px;">
              <el-tag 
...
<template #default="{row}">
  <el-button type="primary" size="small" icon="User" title="分配角色" @click="setRole(row)" ></el-button>
  <el-button type="warning" size="small" icon="Edit" title="编辑" @click="updateUser(row)"></el-button>
  <el-popconfirm 
    ...
    <template #default>
      <el-form >
        ...
        <el-form-item label="职位列表" >
          ...
          <br>
          <el-checkbox-group v-model="userRole" @change="handleCheckChange" style="display: flex; flex-wrap: wrap; gap: 10px;">
            <el-checkbox v-for="(role,index) in allRole" :key="index" :label="role.roleName" ></el-checkbox>
          </el-checkbox-group>
        </el-form-item>
      </el-form>
    </template>
...
import { reqUserInfo,reqAddOrUpdateUser,reqSetUserRole,reqRemoveUser,reqDeleteUserBatch } from '../../../api/acl/user';
import type { Records ,UserResponseData,User,AllRoleResponseData,RoleData,SetRoleData} from '../../../api/acl/user/type';
...
const rules = reactive({
  ...
  name: [{ required: true, message: '请输入用户昵称', trigger: 'blur' ,validator: validateName}],
  password: [{ required: true, message: '请输入用户密码', trigger: 'blur' ,validator: validatePassword}],
})

const getHasUser = async (pager=1) => {
  currentPage.value = pager
  const result: UserResponseData = await reqUserInfo(currentPage.value, pageSize.value, keyword.value)
  if(result.code == 200){
...
const setRole = async (row: User) => {
  ...
  if(result.code == 200){
    ...
    if (row.roleName && row.roleName.trim()) {
      // 将roleName按逗号分割成数组
      userRole.value = row.roleName.split(',').map(role => role.trim()).filter(role => role)
    } else {
      // 用户没有角色
      userRole.value = []
    }
...
const confirm = async () => {
  const data: SetRoleData = {
    ...
    roleIdList: userRole.value.map(roleName => {
  ...
  if(result.code == 200){
    ...
    const userIndex = userArr.value.findIndex(user => user.id === userParams.id)
    if (userIndex !== -1) {
