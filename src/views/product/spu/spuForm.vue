<template>
  ...
  <el-dialog v-model="dialogVisible" title="预览">
   ...
   <el-form-item label="SPU销售属性" >
     ...
     :placeholder="unSelectSaleAttr.length?`还未选择${unSelectSaleAttr.length}个销售属性`:'请先选择销售属性'"
     v-model="saleAttrIdAndName"
     >
            <el-option v-for="item in unSelectSaleAttr"
             :key="item.id"
             :label="item.name" 
        ...
        @click="addSaleAttr"
        :disabled="saleAttrIdAndName?false:true">添加属性值</el-button>
        <el-table border style="margin: 10px 0;" :data="SaleAttr">
            <el-table-column label="序号" type="index" width="80" align="center"></el-table-column>
            <el-table-column label="销售属性名" prop="saleAttrName" width="100"></el-table-column>
            <el-table-column label="销售属性值" prop="attrValue" >
                <template #default="{row}">
                    <el-tag style="margin: 0 5px;" v-for="(item) in row.spuSaleAttrValueList" :key="item.id" class="mx-1" closable  >
...
import type { SpuData ,TradeMark,SaleAttrResponseData,
    HASSaleAttrResponseData,SpuHasImage,
    AllTradeMark,SpuImage,SaleAttr,SaleAttrValue,HasSaleAttr} from '../../../api/product/spu/type'
...
const save = async () => {
    ...
    spuParams.value.spuImageList = SpuImageList.value.map(item=>{
        return {
            imgName:item.name,
            imgUrl:(item as any).response&&(item as any).response.data||item.url
        }
    })
